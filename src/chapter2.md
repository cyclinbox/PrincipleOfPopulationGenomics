...
在具有地理结构的大型种群中，局部杂交单元（local interbreeding unit）具有一定的意义，因为在这样的单元中，适应性进化可以通过系统性地改变等位基因频率而得以发生。这样的局部杂交单元通常被称为 **局部种群（local population）** 或 **区系（demes）** ，并且是群体遗传学的基本单位。局部种群是一个物种的实际进化单位。除非另有规定（或上下文明确暗示），本书中使用的 *种群（population）* 一律代表 *局部种群（local population）* 。局部种群有时也被称为 *孟德尔种群（Mendelian population）* 或 *亚群（subpopulation）* 。
